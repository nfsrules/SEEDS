...
def calculate_inception_stats(
    ...
    for images, _labels in tqdm.tqdm(
        ...
        output = detector_net(images.to(device), **detector_kwargs2).to(torch.float64)
        probs.append(output)

    probs = torch.vstack(probs)  # Tensor([n_images_of_processor_rank_r, n_classes])
    gen_probs = torch.zeros(
        [len(dataset_obj), probs.size(1)], dtype=torch.float64, device=device
    )
    # Calculate grand totals.
    torch.distributed.all_reduce(mu)
    torch.distributed.all_reduce(sigma)
    torch.distributed.all_gather_into_tensor(gen_probs, probs)

    mu /= len(dataset_obj)
    sigma -= mu.ger(mu) * len(dataset_obj)
    sigma /= len(dataset_obj) - 1
    return mu.cpu().numpy(), sigma.cpu().numpy(), gen_probs.cpu().numpy()
...
def calculate_is_from_inception_stats(gen_probs, num_splits=10):
    num_gen = gen_probs.shape[0]  # 50000
    scores = []
...
def calc(image_path, ref_path, num_expected, seed, batch, num_splits):
    """Calculate FID & IS for a given set of images."""
    torch.multiprocessing.set_start_method("spawn")
    dist.init()

    dist.print0(f'Loading dataset ref stats from "{ref_path}"...')
    ref = None
    if dist.get_rank() == 0:
        with dnnlib.util.open_url(ref_path) as f:
            ref = dict(np.load(f))

    mu, sigma, gen_probs = calculate_inception_stats(
    ...
    if dist.get_rank() == 0:
        dist.print0("Calculating FID & IS...")
        fid = calculate_fid_from_inception_stats(
            mu, sigma, ref["mu"], ref["sigma"]
        )
        is_mean, is_std = calculate_is_from_inception_stats(
            gen_probs=gen_probs, num_splits=num_splits
        )
        print(f"FID: {fid:g} | IS: mean={is_mean:g}, std={is_std:g}")
    torch.distributed.barrier()
...
def ref(dataset_path, dest_path, batch):
    ...
    mu, sigma, _ = calculate_inception_stats(
        image_path=dataset_path, max_batch_size=batch
    )
