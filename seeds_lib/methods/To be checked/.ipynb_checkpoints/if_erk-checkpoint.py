...
class IF_ERK(object):
    def __init__(self, model_fn, noise_schedule):
        """Construct (deterministic) IF ERK solvers. 
        Args:
    ...
    def first_update(self, x, s, t, noise_s=None, return_noise=False):
        ...
        x_t = (a1[(...,) + (None,)*dims].mul(x)
                -(a1*sigma_s*h)[(...,) + (None,)*dims].mul(noise_s))
    ...
    def second_update(self, x, s, t, noise_s=None, return_noise=False):
        return x
        """
        A single step for IF ERK2 (TO BE DONE)
        Args:
            x: A pytorch tensor. The initial value at time `s`.
            s: A pytorch tensor. The starting time, with the shape (x.shape[0],).
            t: A pytorch tensor. The ending time, with the shape (x.shape[0],).
    ...
    def third_update(self, x, s, t, r1=1./3., r2=2./3., noise_s=None, noise_s1=None, noise_s2=None):
        return x
        """
        A single step for IF ERK3 (TO BE DONE)
        Args:
            x: A pytorch tensor. The initial value at time `s`.
            s: A pytorch tensor. The starting time, with the shape (x.shape[0],).
            t: A pytorch tensor. The ending time, with the shape (x.shape[0],).
