...
class ETD_ERK_SOLVER():
    def __init__(self, net, lamb, lamb_inv, class_labels, sigma, s, order, c2, noise_pred, butcher_type):
        self.net = net
        self.lamb = lamb
        ...
        if i != num_steps - 1: # exit contidion
            if self.noise_pred: # Noise prediction mode
                ...
                if self.butcher_type == '2a':
                    x_t = self.s(t)/ self.s(s) * x - std(t) * phi_1 * model_s - (0.5 / self.c2) * std(t) * phi_1 * (model_s2 - model_s)
                elif self.butcher_type == '2b':
                    phi_2 = phi_1 / h - 1.
                    x_t = self.s(t)/ self.s(s) * x - std(t) * phi_1 * model_s - (1. / self.c2) * std(t) * phi_2 * (model_s2 - model_s)
            else:
                phi_12 = torch.expm1(-self.c2 * h)
                phi_1 = torch.expm1(-h)                
                model_s = self.net(x / self.s(s), self.sigma(s), self.class_labels).to(torch.float64)                
                ...
                model_s2 = self.net(x_s2 / self.s(s2), self.sigma(s2), self.class_labels).to(torch.float64)                
                if self.butcher_type == '2a':
                    x_t = std(t) / std(s) * x - self.s(t) * phi_1 * model_s - (0.5 / self.c2) * self.s(t) * phi_1 * (model_s2 - model_s)
                elif self.butcher_type == '2b':
                    phi_2 = phi_1 / h + 1.
                    x_t = std(t) / std(s) * x - self.s(t) * phi_1 * model_s + (1. / self.c2) * self.s(t) * phi_2 * (model_s2 - model_s)  
        else:
            x_t = x
            # model_s = self.net(x / self.s(s), self.sigma(s), self.class_labels).to(torch.float64)
            # model_s = n_pred(model_s, x, s)
    ...
    def third_update(self, x, s, t, num_steps, i):
        ...
        if i != num_steps - 1: # exit contidion
            if self.noise_pred: # Noise prediction mode 
                ...
                model_s2 = self.net(x_s2 / self.s(s2), self.sigma(s2), self.class_labels).to(torch.float64)
                model_s2 = n_pred(model_s2, x_s2, s2)                
                x_s3 = self.s(s3) / self.s(s) * x - std(s3) * phi_13 * model_s - c3 / self.c2 * std(s3) * phi_23 * (model_s2 - model_s)
                ...
                if self.butcher_type == '3a':
                    x_t = self.s(t) / self.s(s) * x - std(t) * phi_1 * model_s - 1.5 * std(t) * phi_2 * (model_s3 - model_s)     
                elif self.butcher_type == '3b':
                    assert self.c2 == 1/3
                    D1 = - 1.5 * model_s3 + 6. * model_s2 - 4.5 * model_s
                    D2 = 9. * (model_s3 - 2. * model_s2 + model_s)
                    x_t = self.s(t) / self.s(s) * x - std(t) * phi_1 * model_s - std(t) * phi_2 * D1 - std(t) * phi_3 * D2
            else:
                phi_12 = torch.expm1(-self.c2 * h)
                phi_13 = torch.expm1(-c3 * h)
                phi_1 = torch.expm1(-h)
                ...
                x_s2 = std(s2) / std(s) * x - self.s(s2) * phi_12 * model_s                
                model_s2 = self.net(x_s2 / self.s(s2), self.sigma(s2), self.class_labels).to(torch.float64)                
                x_s3 = std(s3) / std(s) * x - self.s(s3) * phi_13 * model_s + (c3 / self.c2) * self.s(s3) * phi_23 * (model_s2 - model_s)                
                model_s3 = self.net(x_s3 / self.s(s3), self.sigma(s3), self.class_labels).to(torch.float64)

                if self.butcher_type == '3a':
                    x_t = std(t) / std(s) * x - self.s(t) * phi_1 * model_s + 1.5 * self.s(t) * phi_2 * (model_s3 - model_s)
                elif self.butcher_type == '3b':
                    assert self.c2 == 1/3
                    D1 = - 1.5 * model_s3 + 6. * model_s2 - 4.5 * model_s
                    D2 = 9. * (model_s3 - 2. * model_s2 + model_s)
                    x_t = std(t) / std(s) * x - self.s(t) * phi_1 * model_s + self.s(t) * phi_2 * D1 - self.s(t) * phi_3 * D2
        else:
            x_t = x
            # model_s = self.net(x / self.s(s), self.sigma(s), self.class_labels).to(torch.float64)
            # model_s = n_pred(model_s, x, s)
    ...
    def fourth_update(self, x, s, t, num_steps, i):
        ...
        if i != num_steps - 1: 
            if self.noise_pred:
                ...
                model_s2 = self.net(x_s2 / self.s(s2), self.sigma(s2), self.class_labels).to(torch.float64)
                model_s2 = n_pred(model_s2, x_s2, s2)                
                x_s3 = int1 * x - std(s2) * phi_12 * model_s - std(s2) * phi_23 * (model_s2 - model_s)
                model_s3 = self.net(x_s3 / self.s(s2), self.sigma(s2), self.class_labels).to(torch.float64)
                model_s3 = n_pred(model_s3, x_s3, s2)
                x_s4 = int2 * x - std(t) * phi_1 * model_s - std(t) * phi_2 * (model_s2 + model_s3 - (2. * model_s)) 
                model_s4 = self.net(x_s4 / self.s(t), self.sigma(t), self.class_labels).to(torch.float64)
                model_s4 = n_pred(model_s4, x_s4, t)                
                x_s5 = int1 * x - std(s2) * phi_12 * model_s - std(s2) * a_52 * (model_s2 + model_s3 - (2. * model_s)) - std(s2) * a_54 * (model_s4 - model_s)
                model_s5 = self.net(x_s5 / self.s(s2), self.sigma(s2), self.class_labels).to(torch.float64)
                model_s5 = n_pred(model_s5, x_s5, s2)
                x_t = int2 * x - std(t) * phi_1 * model_s - std(t) * phi_2 * (( 4. * model_s5) - model_s4 - (3. * model_s)) - 4. * std(t) * phi_3 * (model_s + model_s4 - (2. * model_s5))
